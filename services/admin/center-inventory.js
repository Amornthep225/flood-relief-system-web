...
async function parseResponse(response) {
    ...
    if (!response.ok) {
        ...
        if (response.status === 404) {
            throw new Error(data?.message || "ไม่พบข้อมูลที่ต้องการ");
        }
...
export async function getCenterById(centerId) {
    ...
    return authorizedFetch(`${API_URL}/centers/${encodeURIComponent(centerId)}`, {
        method: "GET",
    });
}
...
export async function getInventoryTransactions(inventoryId) {
    ...
    return authorizedFetch(
        `${API_URL}/CenterInventories/${encodeURIComponent(
            inventoryId
        )}/transactions`,
        { method: "GET" }
    );
    ...
    const results = await Promise.allSettled(
        validItems.map(async (inventory) => {
            ...
            return list.map((transaction) => ({
                ...transaction,
                centerInventoryId: transaction.centerInventoryId ?? inventory.id,
                reliefItemId: transaction.reliefItemId ?? inventory.reliefItemId,
                reliefItemName: transaction.reliefItemName ?? inventory.reliefItemName,
...
export async function updateInventoryThresholds(
    ...
    if (!Number.isInteger(max) || max < 0) {
        throw new Error("จำนวนสูงสุดต้องเป็นเลขจำนวนเต็มตั้งแต่ 0 ขึ้นไป");
    }

    if (max > 0 && max < min) {
        throw new Error("จำนวนสูงสุดต้องมากกว่าหรือเท่ากับจำนวนขั้นต่ำ");
    }

...
export default function CreateStaffModal({ onClose, onCreated }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await Swal.fire({
                icon: "success",
                title: ui("เพิ่มเจ้าหน้าที่สำเร็จ"),
                timer: 1000,
            ...
            Swal.fire({
                icon: "error",
                title: ui("เพิ่มเจ้าหน้าที่ไม่สำเร็จ"),
                text: err.message,
            });
        } finally {
            setLoading(false);
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm px-4">
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
                <div className="bg-slate-50 p-5 border-b border-slate-100 flex justify-between">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                            {ui("เจ้าหน้าที่ต้องผูกกับศูนย์เพื่อใช้จัดการคลังและภารกิจ")}
                        </p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-3">
                    ...
                    <StaffInput name="phoneNumber" value={form.phoneNumber} onChange={handleChange} placeholder={ui("เบอร์โทร")} />
                    <StaffInput name="password" type="password" value={form.password} onChange={handleChange} placeholder={ui("รหัสผ่าน")} />
                    ...
                    <div className="flex gap-3 pt-4">
                        ...
                        >
                            {loading ? ui("กำลังบันทึก...") : ui("บันทึก")}
                        </button>
                    </div>
                </form>
            </div>

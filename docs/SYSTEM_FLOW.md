...
## Requirement 11 — Admin จัดการศูนย์เดียว

เปิด `/admin/admin-centers` แล้วโหลดศูนย์จาก `GET /api/Centers` ผ่าน service โดยตรง
เมื่อมีศูนย์หนึ่งรายการจะแสดงฟอร์มแก้ไขทันที รวมถึงศูนย์ที่ปิดใช้งาน
บันทึกผ่าน `PUT /api/Centers/{id}` โดยใช้ ID เดิม และสามารถคืนค่าฟอร์มกลับไปยังค่าที่บันทึกไว้ได้
ปุ่มจัดการคลังส่ง centerId ของศูนย์นั้นไปยัง `/admin/admin-center-inventory`
หากไม่มีศูนย์จะแสดงข้อความให้ตั้งค่าข้อมูลก่อน หากมีหลายศูนย์จะแสดงข้อผิดพลาดให้ตรวจสอบข้อมูล ไม่เลือกหรือลบศูนย์อัตโนมัติ
หน้านี้ไม่มีขั้นตอนเลือกรายการศูนย์หรือปุ่มเพิ่ม/ลบศูนย์ ไม่ต้องใช้ migration เพิ่ม
